...
static void d4_gpio_key_init(void){
	...
	drime4_gpio_key_buttons[6].gpio = GPIO_REC_KEY;
	drime4_gpio_key_buttons[7].gpio = WIFI_PWR_CNT;
}
...
static struct adc_info_key adc0_key[4]={
	{.code = KEY_F1, .adc_level = 53, }, //lens
	{.code = KEY_F3, .adc_level = 150, }, //wifi
	{.code = KEY_F4, .adc_level = 101, }, //scene
	{.code = KEY_F10, .adc_level = 0, }, //m
};

static struct adc_info_key adc7_key[4]={
	{.code = KEY_F6, .adc_level = 0, }, //smart
	{.code = KEY_F7, .adc_level = 53, }, //p
	{.code = KEY_F8, .adc_level = 101, }, //a
	{.code = KEY_F9, .adc_level = 150, }, //s
};

static struct adc_info_key adc1_key[5]={
	{.code = KEY_KP2, .adc_level = 75, 	.mask_id = MASK_DOWN, }, //down
	{.code = KEY_KP4, .adc_level = 35, 	.mask_id = MASK_LEFT, }, //left
	{.code = KEY_KP6, .adc_level = 155, 	.mask_id = MASK_RIGHT, }, //right
	{.code = KEY_KP8, .adc_level = 0, 		.mask_id = MASK_UP, }, //up
	{.code = KEY_KPENTER, .adc_level = 115, 	.mask_id = MASK_OK, }, //ok
};

static struct adc_info_key adc2_key[5]={
	{.code = KEY_KPDOT, .adc_level = 115, 	.mask_id = MASK_DELET, }, //delete
	{.code = KEY_COMPOSE, .adc_level = 35, 	.mask_id = MASK_MENU, }, //menu
	{.code = KEY_CONFIG, .adc_level = 75, 	.mask_id = MASK_PB, }, //pb
	{.code = KEY_HOMEPAGE, .adc_level = 155, 	.mask_id = MASK_FN, }, //func
	{.code = KEY_REFRESH, .adc_level = 0, 	.mask_id = MASK_EV, }, //av
};

static void d4keys_polled_init(void)
{
	drime4_key_buttons_polled[0].key_info = adc1_key;
	drime4_key_buttons_polled[1].key_info = adc2_key;
	drime4_key_buttons_polled[2].key_info = adc0_key;
	drime4_key_buttons_polled[3].key_info = adc7_key;

	drime4_key_buttons_polled[0].key_cnt = ARRAY_SIZE(adc1_key);
	drime4_key_buttons_polled[1].key_cnt = ARRAY_SIZE(adc2_key);
	drime4_key_buttons_polled[2].key_cnt = ARRAY_SIZE(adc0_key);
	drime4_key_buttons_polled[3].key_cnt = ARRAY_SIZE(adc7_key);
...
static int tsu6111a_attach_callback(int dev_type)
{
	...
	case MUSB_DEV_USB:
		...
		reset_flag = 0;
		timerflag = 0;
        jack_event_handler("usb_discont", 4);
        mdelay(10);
...
//		jack_event_handler("release", 1);
		break;
	case MUSB_DEV_CHARGER:
		printk(KERN_EMERG"charger1 1\n");
        jack_event_handler("charger1", 1);
		break;		
	default:
		printk("not surport\n");
		break;
...
static int tsu6111a_detach_callback(int dev_type)
{
	...
	case MUSB_DEV_USB:
#ifndef CONFIG_USBSTARTSTOP_SYSFS
		{
			...
			gpio_free(GPIO_USB_DET);
			pinctrl_free_gpio(GPIO_USB_DET);
            jack_event_handler("usb_discont", 0);
            mdelay(10);
			jack_event_handler("usb", 0);
		}
#else
...
//		jack_event_handler("release", 0);
		break;
	case MUSB_DEV_CHARGER:
		printk(KERN_EMERG"charger1 0\n");
        jack_event_handler("charger1", 0);
		break;
	default:
		printk("not surport\n");
		break;
...
static struct wm8994_retune_mobile_cfg wm8994_retune_eq[] = {
	{   //[0]
		.name = "NX300 EQ2050 (new)",
		.rate = 48000,
		.regs = {
				0x3804, 0x2b00, 0x0F2F, 0x0400, 0x0344, 
				0x1c4f, 0xf103, 0x040b, 0x0402, 0x1523,
				0xf069, 0x040d, 0x01a0, 0x09c4, 0xf066,
				0x0407, 0x0194, 0x0000, 0x0400, 0x4000
		},
	},
	{   //[1]
		.name = "NX300 EQ2050 (old)",
		.rate = 48000,
		.regs = {
	...
	{   //[3]
		.name = "NX300 EQ for external mic",
		.rate = 32000,
		.regs = {
			0x4004, 0x2b00, 0x0ECA, 0x03FF, 0x04D8, 
			0x18AE, 0xf17F, 0x040A, 0x05ED, 0x09B3,
			0xf09D, 0x040a, 0x026E, 0xF64B, 0xf098,
			0x0407, 0x025A, 0xF95F, 0x0400, 0x257C

		},
	},
	{   //[4]
		.name = "NX300 general EQ",
		.rate = 48000,
		.regs = {
	...
	{   //[7]
		.name = "NX300 EQ1855 for external mic",
		.rate = 32000,
		.regs = {
...
static struct wm8994_drc_cfg wm8994_retune_drc[] = {
	{   //[0]
		.name = "NX300 DRC params for internal mic: lens 20-50",
		.regs = {
            0x0710, 0x0453, 0x0818, 0x047e, 0x0140
		},
	},
	{   //[1]
		.name = "NX300 DRC2050 (old)",  /* slope 4 under -52.5 dB */
		.regs = {
				0x0710, 0x0e53, 0x0808, 0x0232, 0x0160
		},
	},
	{   //[2]
		.name = "NX300 DRC params for internal mic: lens 18-55",
		.regs = {
            0x0710, 0x0453, 0x0818, 0x047e, 0x0140
		},
	},
	{   //[3]
		.name = "NX300 DRC params for external mic: lens 20-50",
		.regs = {
                0x0710, 0x0453, 0x0818, 0x035b, 0x0120
		},
	},
	{   //[4]
		.name = "NX300 general DRC for internal mic",
		.regs = {
            0x0710, 0x0453, 0x0818, 0x047e, 0x0140
		},
	},
	{   //[5]
		.name = "NX300 DRC params for external mic: lens 18-55",
		.regs = {
                0x710, 0x453, 0x818, 0x035b, 0x120
		},
	},
	{   //[6]
		.name = "NX300 DRC params for intrnal mic: lens 20-50 (loud env)",
		.regs = {
                0x193, 0x453, 0x809, 0x1e9, 0x320
		},
	},
...
static void __init drime4_init_i2c(void)
{
	...
	if(GetBoardVersion() >= eBdVer_PR1B){
	...
	i2c_register_board_info(2, i2c_devs2, size);
#endif
...
MACHINE_START(D4ES, "Samsung-DRIMeIV-NX300")
/* Maintainer: */
	.atag_offset	= 0x100,
