...
int gpu_init_flag = 0;
int fb_init_flag = 0;
static struct drime4_gpu g_gpu;
...
#ifdef DRIME4_SLP
	/* bh1212 DRIME4_GPU_CLK_ENABLE */
	g_gpu.dev = &pDevice->dev;

	g_gpu.clock = clk_get(&pDevice->dev, "gpu");
	clk_enable(g_gpu.clock);

	/* bh1212 DRIME4_GPU_LCD_ON */
//	gpu_init_flag = 1;
//	if (fb_init_flag == 1) {
		D4dpLcdBridgeInit();
//	}
...
#ifdef DRIME4_SLP
	/* bh1212 DRIME4_GPU_CLK_DISABLE */
	clk_disable(g_gpu.clock);
	clk_put(g_gpu.clock);
	D4dpLcdBridgeExit();
#endif
