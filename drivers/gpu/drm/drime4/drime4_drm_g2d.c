...
struct drm_drime4_pending_g2d_event {
	struct drm_pending_event	base;
	struct drm_drime4_g2d_event	event;
};
...
struct g2d_cmdlist_node {
	...
	struct drm_drime4_pending_g2d_event	*event;
};
...
static void g2d_add_cmdlist_to_inuse(struct drime4_drm_g2d_private *g2d_priv,
				     struct g2d_cmdlist_node *node)
{
...
static int g2d_get_cmdlist_gem(struct drm_device *drm_dev,
			       ...
			       struct g2d_cmdlist_node *node)
{
	struct drm_drime4_file_private *file_priv = file->driver_priv;
	struct drime4_drm_g2d_private *g2d_priv = file_priv->g2d_priv;
	...
	for (i = 0; i < node->gem_nr; i++) {
		...
		addr = drime4_drm_gem_get_dma_addr(drm_dev, gem_node->handle,
						   file);
		if (IS_ERR(addr)) {
			node->gem_nr = i;
			kfree(gem_node);
				...
				unsigned int nr)
{
	...
	list_for_each_entry_safe_reverse(node, n, &g2d_priv->gem_list, list) {
		...
		drime4_drm_gem_put_dma_addr(drm_dev, node->handle, file);
		list_del_init(&node->list);
		kfree(node);
		nr--;
	}
...
static void g2d_finish_event(struct g2d_data *g2d, u32 cmdlist_no)
{
	...
	struct drm_drime4_pending_g2d_event *e;
	struct timeval now;
	unsigned long flags;
	...
	e = list_first_entry(&runqueue_node->event_list,
			     struct drm_drime4_pending_g2d_event, base.link);
...
/* ioctl functions */
int drime4_g2d_get_ver_ioctl(struct drm_device *drm_dev, void *data,
			     struct drm_file *file)
{
	struct drm_drime4_g2d_get_ver *ver = data;
...
EXPORT_SYMBOL_GPL(drime4_g2d_get_ver_ioctl);

int drime4_g2d_set_cmdlist_ioctl(struct drm_device *drm_dev, void *data,
				 struct drm_file *file)
{
	struct drm_drime4_file_private *file_priv = file->driver_priv;
	struct drime4_drm_g2d_private *g2d_priv = file_priv->g2d_priv;
	...
	struct g2d_data *g2d;
	struct drm_drime4_g2d_set_cmdlist *req = data;
	struct drm_drime4_g2d_cmd *cmd;
	struct drm_drime4_pending_g2d_event *e;
	struct g2d_cmdlist_node *node;
	struct g2d_cmdlist *cmdlist;
	unsigned long flags;
	...
	if (req->event_type != G2D_EVENT_NOT) {
		...
		e->event.base.type = DRM_DRIME4_G2D_EVENT;
	...
	cmd = (struct drm_drime4_g2d_cmd *)(uint32_t)req->cmd;
	...
	if (req->cmd_gem_nr) {
		struct drm_drime4_g2d_cmd *cmd_gem;

		cmd_gem = (struct drm_drime4_g2d_cmd *)(uint32_t)req->cmd_gem;
...
EXPORT_SYMBOL_GPL(drime4_g2d_set_cmdlist_ioctl);

int drime4_g2d_exec_ioctl(struct drm_device *drm_dev, void *data,
			  struct drm_file *file)
{
	struct drm_drime4_file_private *file_priv = file->driver_priv;
	struct drime4_drm_g2d_private *g2d_priv = file_priv->g2d_priv;
	struct device *dev = g2d_priv->dev;
	struct g2d_data *g2d;
	struct drm_drime4_g2d_exec *req = data;
	struct g2d_runqueue_node *runqueue_node;
	struct list_head *run_cmdlist;
	struct list_head *event_list;
...
EXPORT_SYMBOL_GPL(drime4_g2d_exec_ioctl);
...
static int __devinit g2d_probe(struct platform_device *pdev)
{
	...
	ret = drime4_drm_subdrv_register(subdrv);
	if (ret < 0) {
		dev_err(dev, "failed to register drm g2d device\n");
		goto err_free_irq;
	}

	dev_info(dev, "The drime4 g2d(ver %d.%d) successfully probed\n",
...
static int __devexit g2d_remove(struct platform_device *pdev)
{
	...
	drime4_drm_subdrv_unregister(&g2d->subdrv);
	free_irq(g2d->irq, g2d);
