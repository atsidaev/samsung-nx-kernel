...
#define IOCTL_MFC_HW_INIT			(0x00800005)
#define IOCTL_MFC_FB_STATE			(0x00800006)
...
struct mfc_enc_exe_arg {
	...
	unsigned int in_strm_size; /*[IN]Out-buffer size of encoded strm */
...
//	u32  convert_buf_y_addr;
//	u32  convert_buf_c_addr;
};
...
struct mfc_dec_desc_input_buf {
	unsigned int start_addr;
	unsigned int size;
	unsigned int align;
};

struct mfc_buf_cma_base {
	unsigned int base0;
	unsigned int base1;
	struct mfc_dec_desc_input_buf desc_input_buf;
//	unsigned int reserved_size_port0;
//	unsigned int reserved_size_port1;
};
...
union mfc_args {
	...
	*/
	...
	/* get CMA reserved address from user */
	struct mfc_buf_cma_base cma_base;
	/* set buf addr for tile2linear convert */
	struct mfc_buf_t2l t2l_buf;
};

struct mfc_common_args {
	enum mfc_ret_code ret_code;	/* [OUT] error code */
	enum fb_state fb_state;
	enum nlc_state nlc_state;
	union mfc_args args;
	/* input buffer reordering */
	ENC_YUV_INFO encoder_input_info[ENC_INPUT_BUF_NO];
};
...
typedef struct
{
	...
	int input_secure_id;
	int input_size;
	/* added for framebuffer input buffer reordering */
	ENC_YUV_INFO encoder_input_info[ENC_INPUT_BUF_NO];
	unsigned int current_frame_num;
} _MFCLIB;
