...
#define ADDR_REF_C(i)	ctx->enc_ref_buf_c##i##_addr
...
int h264_get_init_arg(struct mfc_inst_ctx *ctx, void *arg)
{
	...
	ctx->numofbframe = init_h264_arg->in_bframenum;
	ctx->period_of_iframe = init_arg->cmn.in_gop_num;

	mfc_dbg("MFC_ENC_PIC_TYPE_CTRL = 0x%x\n", reg);
...
static int set_init_arg(struct mfc_inst_ctx *ctx, void *arg)
{
	...
	init_arg->cmn.out_buf_size.strm_ref_y = enc_ctx->streamsize;
...
int set_strm_ref_buf(struct mfc_inst_ctx *ctx)
{
	...
	*/
	...
	enc_ctx->chromasize = ALIGN(ctx->width + 16, ALIGN_W) \
		* ALIGN((ctx->height >> 1) + 4, ALIGN_H);
	...
	for (i = 0; i < 2; i++) {
		...
		/*
		 * we need to save recon buffer for further use
		 */
		if (i == 0)
			ctx->enc_ref_buf_y0_addr = alloc->real;
		else
			ctx->enc_ref_buf_y1_addr = alloc->real;
		...
		if (i == 0)
			ctx->enc_ref_buf_y2_addr = alloc->real;
		else
			ctx->enc_ref_buf_y3_addr = alloc->real;
	}

	mfc_dbg("instance id =%d\n", ctx->id);
	mfc_dbg("y0 addr = 0x%x\n", ctx->enc_ref_buf_y0_addr);
	mfc_dbg("y1 addr = 0x%x\n", ctx->enc_ref_buf_y1_addr);
	mfc_dbg("y2 addr = 0x%x\n", ctx->enc_ref_buf_y2_addr);
	mfc_dbg("y3 addr = 0x%x\n", ctx->enc_ref_buf_y3_addr);
	...
	for (i = 0; i < 4; i++) {
		...
		if (i == 0)
			ctx->enc_ref_buf_c0_addr = alloc->real;
		else if(i == 1)
			ctx->enc_ref_buf_c1_addr = alloc->real;
		else if(i == 2)
			ctx->enc_ref_buf_c2_addr = alloc->real;
		else if(i == 3)
			ctx->enc_ref_buf_c3_addr = alloc->real;

//		ADDR_REF_C(i) = alloc->real;
	}
...
int mfc_init_encoding(struct mfc_inst_ctx *ctx, union mfc_args *args)
{
	...
	/* ctx->codec_type is going to be used in mfc_fb_init() */
	ctx->codec_type = init_arg->cmn.in_codec_type;
	...
	/*if (ctx->width >= 1920 || ctx->height >= 1080)
        write_shm(ctx, 1, 0x110);
    else*/
        write_shm(ctx, 0, 0x110);
...
static int mfc_encoding_frame(struct mfc_inst_ctx *ctx, struct mfc_enc_exe_arg *exe_arg)
{
	 ...
	 */
	/* future work
	 * if user handles buffers more than one,
	 * driver must set the changed location and size of the stream buffer every frame
	 *  based on user arguments.
	 */
	write_reg(mfc_mem_base_ofs(exe_arg->in_strm_st) >> 11, MFC_ENC_SI_CH1_SB_ADR);
	if (exe_arg->in_strm_size == 0)
		ret = MFC_INVALID_PARAM_FAIL;
	else
		write_reg(exe_arg->in_strm_size, \
			MFC_ENC_SI_CH1_SB_SIZE);
