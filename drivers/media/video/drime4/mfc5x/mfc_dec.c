...
static void dump_stream(unsigned long address, unsigned int size)
{
	...
	mfc_info("---- start stream dump ----\n");
	mfc_info("size: 0x%04x\n", size);
	for (i = 0; i < size; i += 16) {
		mfc_dbg("0x%04x: ", i);
...
static int h264_post_seq_start(struct mfc_inst_ctx *ctx)
{
	...
	*/
	...
	/* only 264 is available */
	ctx->dpb_y = kmalloc(dec_ctx->numtotaldpb * sizeof(u32), GFP_KERNEL);
	ctx->dpb_c = kmalloc(dec_ctx->numtotaldpb * sizeof(u32), GFP_KERNEL);
...
static int h264_set_dpbs(struct mfc_inst_ctx *ctx)
{
	...
	/* for multi instance decoding with FB on */
	u32 *dpb_luma = ctx->dpb_y;
	u32 *dpb_chroma = ctx->dpb_c;
	...
	if (ctx->dec_out_buf_y_addr[0] != NULL) {
		for (i = 0; i < 6; i++) {
			/*
			 * set chroma buffer address
			 */
			write_reg(mfc_mem_base_ofs(ctx->dec_out_buf_y_addr[i]) >> 11, MFC_CHROMA_ADR + (4 * i));
			/*
			 * set luma buffer address
			 */
			write_reg(mfc_mem_base_ofs(ctx->dec_out_buf_y_addr[i]) >> 11, MFC_LUMA_ADR + (4 * i));
			/*
			 * allocate mv buffer
			 */
	...
	} else if (ctx->dec_out_buf_y_addr[0] == NULL) {
		for (i = 0; i < dec_ctx->numtotaldpb; i++) {
			/*
			 * allocate chroma buffer
			 */
			...
			*(dpb_chroma + i) = alloc->real;
			...
			/* for multi instance decoding with FB on */
			*(dpb_luma + i) = alloc->real;
			mfc_dbg("dpb_luma %dth buf addr = 0x%x\n", i, *(dpb_luma + i));

			/* for first frame, recon buffer is initialized to first dpb */
			ctx->rec_for_cur_frame_y = *(dpb_luma);
			ctx->rec_for_cur_frame_y = *(dpb_chroma);
...
//
//	/*
//	 * extra buffer for tile2linear converting
//	 * allocate chroma buffer
//	 */
//
//	if (ctx->fb_state == FB_OFF) {
//		alloc = _mfc_alloc_buf(ctx, dec_ctx->chromasize, ALIGN_2KB, MBT_DPB | PORT_A);
//		if (alloc == NULL) {
//		mfc_err("failed alloc chroma buffer\n");
//
//		return -1;
//		}
//		ctx->convert_buf_c_addr = alloc->real;
//		//printk("%s: convert_buf_y = 0x%x\n", alloc->real);
//
//		/*
//		 * extra buffer for tile2linear converting
//		 * allocate luma buffer
...
//			return -1;
//		}
//		ctx->convert_buf_y_addr = alloc->real;
//	}
...
int mfc_init_decoding(struct mfc_inst_ctx *ctx, union mfc_args *args)
{
	...
	/* the addresses should be physical/dma-able */
	for (i = 0; i < 6; i++) {
		ctx->dec_out_buf_y_addr[i] = init_arg->dec_out_buf_y_addr[i];
		ctx->dec_out_buf_c_addr[i] = init_arg->dec_out_buf_c_addr[i];
	}
	/* user let driver knows the buffer addr for converting operation */
//	if (ctx->fb_state == FB_OFF) {
//		ctx->convert_buf_y_addr = init_arg->convert_buf_y_addr;
//		ctx->convert_buf_c_addr = init_arg->convert_buf_c_addr;
//	}
	...
	if (mfc_fb_init(ctx) < 0) {
		mfc_err("FB Initialization Failed");
		ret = MFC_FB_INIT_FAIL;
		goto err_handling;
	}
...
err_handling:
	...
	/* if (!dec_ctx) was orginal code, bug fix is done */
	if (dec_ctx) {
...
static int mfc_decoding_frame(struct mfc_inst_ctx *ctx, \
	struct mfc_dec_exe_arg *exe_arg, int *consumed)
{
	...
	int ret;
	unsigned int flag, recon_index;

	struct mfc_dec_ctx *dec_ctx = (struct mfc_dec_ctx *)ctx->c_priv;
	inSeqId = dec_ctx->frametag;

	recon_index = ctx->current_recon_index;
	flag = 0;
	...
	/* FIXME: */
	/* Force MFC to use the calculated buffer as recon buffer */
	if (ctx->fb_state == FB_ON ) {
		flag |= (1<<recon_index);
		write_reg(flag, MFC_SI_CH1_RELEASE_BUF);
		mfc_dbg("release buf = 0x%x\n", flag);
	} else if (ctx->fb_state == FB_OFF) {
	/* in case fb off, we release all dpbs so that MFC can decide recon
	 * buffer base on its own way
	 */
	...
	if (dec_ctx->immediatelydisplay){
		...
		if (dec_ctx->decstatus == DEC_S_DD) {
			mfc_dbg("[mfc_decoding_frame] \
				ImmediatelyDisplay Status DEC_S_DD\n");
	...
	/* displayable buffer should be locked
	 * until user release it
	 */
//	list_for_each(pos, &mfc_alloc_head[1]) {
//		if (dec_ctx->dispstatus == DISP_S_DECODING)
//			break;
//		alloc = list_entry(pos, struct mfc_alloc_buffer, list);
//		if ((out_display_Y_addr << 11) == alloc->real) {
//			/* do something */
//			ctx->dec_dst_flag &= ~(1 << (i/2)) ;
//			/* set_bit(i, &ctx->dec_dst_flag); */
//			printk("%s: [%d]th buffer is ready to lock\n", \
//				__func__, i/2);
//			break;
//		}
//		i++;
...
//	}
	...
	/* if FB is off, it is neccessary to use h/w converter */
//	if(ctx->fb_state == FB_OFF && dec_ctx->dispstatus != DISP_S_DECODING) {
//		mfc_fb_convert_4x2tile2linear(ctx, (out_display_Y_addr << 11), \
//			(out_display_C_addr << 11), ctx->convert_buf_y_addr, \
//			ctx->convert_buf_c_addr);
//
//		exe_arg->out_display_Y_addr = ctx->convert_buf_y_addr;
//		exe_arg->out_display_C_addr = ctx->convert_buf_c_addr;
//		exe_arg->out_y_offset = mfc_mem_data_ofs(ctx->convert_buf_y_addr, 1);
//		exe_arg->out_c_offset = mfc_mem_data_ofs(ctx->convert_buf_c_addr, 1);
//	} else {
		exe_arg->out_display_Y_addr = (out_display_Y_addr << 11);
		exe_arg->out_display_C_addr = (out_display_C_addr << 11);
		exe_arg->out_y_offset = \
			mfc_mem_data_ofs((out_display_Y_addr << 11), 1);
		exe_arg->out_c_offset = \
			mfc_mem_data_ofs((out_display_C_addr << 11), 1);
//	}
	...
	ctx->current_dec_out_buf_y = out_display_Y_addr;
	ctx->current_dec_out_buf_c = out_display_C_addr;

	/* 2012-01-04
	 * Firmware doesn't write the frame type after decoding at this time
	 */
	exe_arg->out_disp_pic_frame_type  = read_reg(MFC_SI_FRAME_TYPE) & DEC_FRM_MASK;
