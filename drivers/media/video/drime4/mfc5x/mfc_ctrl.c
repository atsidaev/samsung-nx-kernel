...
int mfc_load_firmware(const unsigned char *data, size_t size)
{
	...
	/* we do not use port0 base address */
	/* port0 base address have an offset (fw_size + input buffer size) */
	fw = mfc_mem_addr(0);
...
//		mfc_clock_off();
	...
	/* 2011-06-10  FIXME: Dual bank */
	write_reg(0x1, DRIME4_FB_DUAL_BANK);

	mfc_init_memctrl();

	/* 1-1. Set base address: 0xc8000000 ~ 128MB */
	write_reg((0x1 << 25), DRIME4_FB_CODEC_TOP_CTRL);
