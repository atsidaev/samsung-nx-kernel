...
#endif
/* FIXME exclude following line 2011-08-11 */
...
unsigned int mfc_cal_export_base(int port, int size)
{
	if ((port < 0) || (port >= mem_ports))
		return 0;

	if (port == 0)
		mem_infos[port].export_base = mem_infos[port].base \
				+ MFC_FW_SYSTEM_SIZE + size;
	else
		printk("invalid port num\n");

	return mem_infos[port].export_base;
}
...
unsigned int mfc_mem_data_base(int port)
{
	...
	if (port == 0)
		addr = mem_infos[port].export_base;
	else
...
unsigned int mfc_cal_export_size(int port, int size)
{
	if ((port < 0) || (port >= mem_ports))
		return 0;

	if (port == 0)
		mem_infos[port].export_size = mem_infos[port].size \
				- MFC_FW_SYSTEM_SIZE - size;
	else
		printk("invalid port num\n");

	return mem_infos[port].export_size;
}
...
unsigned int mfc_mem_data_size(int port)
{
	...
	if (port == 0)
		size = mem_infos[port].export_size;
	else
...
int mfc_init_mem_mgr(struct mfc_dev *dev, union mfc_args *args)
{
	...
	int i;
	struct mfc_buf_cma_base *cma_base = (struct mfc_buf_cma_base *)args;
...
#else	/* not SYSMMU_MFC_ON */
	/* early allocator */
#if defined(CONFIG_DRIME4_MEM_CMA)
	if (dev->mem_ports == 1) {
		...
		/* This assumes that the start address of input buffer is same with 
		 * the start addr of port0
		 */
//		base = cma_base->desc_input_buf.start_addr + cma_base->desc_input_buf.size;
		base = cma_base->base0;
