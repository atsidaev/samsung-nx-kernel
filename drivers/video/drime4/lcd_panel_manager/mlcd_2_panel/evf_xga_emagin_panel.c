...
/* Private function */
static int mlcd_2_set_single_table_value(const unsigned char addr, const unsigned char data);
static int mlcd_2_set_burst_table_value(const unsigned char *wbuf, unsigned int param_cnt);
static int mlcd_2_read_single_table_value(const unsigned char addr, unsigned char *data);

/**< LCD panel control functions */
static void mlcd_2_ctrl_power_on(void);
static void mlcd_2_ctrl_reset(void);
static void mlcd_2_ctrl_init(void);
static void mlcd_2_ctrl_display_on(void);
static void mlcd_2_ctrl_display_off(void);
static void mlcd_2_ctrl_standby_on(void);
static void mlcd_2_ctrl_standby_off(void);
static void mlcd_2_ctrl_power_off(void);
static void mlcd_2_ctrl_brightness(int level);
static void mlcd_2_ctrl_extension(void *ext_info);
...
/******************************************************************************/
/*                     Main LCD 2 Panel Information                           */
/******************************************************************************/

struct lcd_panel_hw_info mlcd_2_panel_hw_info = {
...
struct lcd_panel_timing_info mlcd_2_panel_timing_info = {
...
struct lcd_panel_ctrl_func mlcd_2_panel_ctrl_func = {

		/* Basic Function */

		mlcd_2_ctrl_reset, 			/**< LCD reset function */

		mlcd_2_ctrl_power_on,		/**< LCD power on function */
		mlcd_2_ctrl_power_off,		/**< LCD power off function */
		mlcd_2_ctrl_init,			/**< LCD initialization function */

		mlcd_2_ctrl_display_on,		/**< LCD light on  function, including back light on */
		mlcd_2_ctrl_display_off,	/**< LCD light off function, including back light off */

		mlcd_2_ctrl_brightness,		/**< LCD panel brightness setting function */

		/* Optional Function */

		mlcd_2_ctrl_standby_on,     /**< LCD stand-by on Function */
		mlcd_2_ctrl_standby_off,   	/**< LCD stand-by off Function */
		...
		mlcd_2_ctrl_extension,		/**< LCD extension control function */

		NULL,						/**< suspend function for power management */
		NULL						/**< resume  function for power management */
};

struct lcd_panel_manager_info mlcd_2_panel_info = {
    &mlcd_2_panel_hw_info,
    &mlcd_2_panel_timing_info,
    &mlcd_2_panel_ctrl_func,
    " eMagin XGA EVF Panel "
};

/******************************************************************************/
/*                  Main LCD 2 Panel Control Functions                        */
/******************************************************************************/

static void mlcd_2_ctrl_power_on(void)
{
	/**< for test */
	mlcd_2_panel_set_gpio(LPM_GPIO_POWER_1, LPM_ON);
	mdelay(25);
}
...
{
	/**< reset */
	mlcd_2_panel_set_gpio(LPM_GPIO_RESET, LPM_ON);
	mdelay(10);
	mlcd_2_panel_set_gpio(LPM_GPIO_RESET, LPM_OFF);
	mdelay(10);
	mlcd_2_panel_set_gpio(LPM_GPIO_RESET, LPM_ON);
	mdelay(10);
}
...
static void mlcd_2_ctrl_init(void)
{
	...
	/**< table read */
	for (i = 0; i < 20; i++) {
		mlcd_2_read_single_table_value(i, &read_value);
	}
#endif
	...
	mlcd_2_set_burst_table_value(init_burst_table_value_1, set_count);

	set_count = sizeof(init_burst_table_value_2);
	mlcd_2_set_burst_table_value(init_burst_table_value_2, set_count);
	...
	mlcd_2_set_single_table_value(init_value[i][0], init_value[i][1]);
	}
	#endif

#else
	mlcd_2_set_single_table_value(0x27, 0x0F);
	mlcd_2_set_single_table_value(0x21, 0x03);
	mlcd_2_set_single_table_value(0x1A, 0x20);
	mlcd_2_set_single_table_value(0x1B, 0x64);
	mlcd_2_set_single_table_value(0x12, 0x22);
	mlcd_2_set_single_table_value(0x17, 0x0D);
	mlcd_2_set_single_table_value(0x14, 0x05);
	mlcd_2_set_single_table_value(0x01, 0x03);

	/**< VBP: 현재는 설정 하지 않아도 동작 */
	mlcd_2_set_single_table_value(0x07, 0x13);
	/**< HBP: 현재는 설정 하지 않아도 동작 */
	mlcd_2_set_single_table_value(0x08, 0x52);

#endif

	mdelay(10);
	mlcd_2_ctrl_standby_off();
}
...
static void mlcd_2_ctrl_display_on(void)
{
	...
	mlcd_2_set_single_table_value(0x02, 0x00);
	mdelay(10);
}
...
{
	mdelay(10);
	mlcd_2_set_single_table_value(0x2, 0x80);
}

static void mlcd_2_ctrl_power_off(void)
{
	mlcd_2_ctrl_standby_on();
	mlcd_2_ctrl_display_off();
}
...
static int mlcd_2_set_single_table_value(const unsigned char addr, const unsigned char data)
{
	...
	mlcd_2_panel_set_gpio(LPM_GPIO_POWER_3, LPM_ON);
	udelay(10);
	mlcd_2_panel_set_gpio(LPM_GPIO_POWER_3, LPM_OFF);

	ret = mlcd_2_panel_spi_write(&spi_set_info);
	mlcd_2_panel_set_gpio(LPM_GPIO_POWER_3, LPM_ON);
	udelay(10);
...
static int mlcd_2_read_single_table_value(const unsigned char addr, unsigned char *data)
{
	...
	ret = mlcd_2_panel_spi_rw(&spi_set_info);
	mlcd_2_panel_set_gpio(LPM_GPIO_POWER_3, LPM_ON);
...
static int mlcd_2_register(void)
{
#if 1
	...
	if (mlcd_2_panel_set_info(&mlcd_2_panel_info) < 0) {
		return -EINVAL;
	}
	printk("Main LCD 2 panel register \n");
	return 0;
}

static void mlcd_2_unregister(void)
{
	printk("Main LCD 2 panel unregister \n");
}

module_init(mlcd_2_register);
module_exit(mlcd_2_unregister);
