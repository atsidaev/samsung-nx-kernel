...
void dp_pm_request_stop(struct stfb_info *info)
{
	unsigned int reg = 0;
	D4_DP_COMMON_PM_REQUEST_SET(reg, DP_ON);
	__raw_writel(reg, info->dp_global_regs + 0x145c);
}

void dp_pm_request_clear(struct stfb_info *info)
{
	unsigned int reg = 0;
	D4_DP_COMMON_PM_REQUEST_SET(reg, DP_OFF);
	__raw_writel(reg, info->dp_global_regs + 0x145c);
}
#endif
