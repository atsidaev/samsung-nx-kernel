...
#include "../lcd_panel_manager/d4_lcd_panel_manager_if.h"
...
struct stfb_lcd_pannel lcd_pannel = {0,};
struct stfb_lcd_pannel sublcd_pannel = {0,};
...
static unsigned int lcd_out_type;
static int g_dp_lcd_interrupt_state;
static int g_dp_lcd_interrupt_arg[MAX_INTERRUPT];
		...
		struct stdp_lcd_tg tg)
{
	...
	reg = 0;
	D4_DP_LCD_FRAME_INIT_LINE(reg, tg.enable_v_end + 1);
	D4_DP_LCD_FRAME_FINAL_LINE(reg, tg.enable_v_end);
	__raw_writel(reg, info->dp_lcd_regs + DP_LCD_TG_INFO0);

	reg = 0;
	D4_DP_LCD_FIELD_INIT_LINE(reg, tg.enable_v_end + 1);
		...
		enum edp_video_bit vid_bit, struct stdp_display_area *display)
{
	...
	if (h_end == 480) {		/* for amoled lcd green line */
		h_end -= 2;
	}
...
/**
 * @brief boundary box rectangle style
...
/**
 * @brief boundary box select color table
...
/**
 * @brief boundary box dislpay area
...
static void dp_lcd_out_type(struct stfb_info *info, enum elcd_out_type type)
{
	...
	case LCD_OUT_BT656_8BIT_CbYCrY:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_ON);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_ON);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_24);
		D4_DP_LCD_OUT_BT656_BITSEL(reg0, DP_OFF);
		D4_DP_LCD_OUT_BT656_CBCR_SEL(reg0, DP_OFF);
		break;

	case LCD_OUT_BT656_8BIT_CrYCbY:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_ON);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_ON);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_24);
		D4_DP_LCD_OUT_BT656_BITSEL(reg0, DP_OFF);
		D4_DP_LCD_OUT_BT656_CBCR_SEL(reg0, DP_ON);
		break;

	case LCD_OUT_BT656_10BIT_CbYCrY:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_ON);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_ON);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_24);
		D4_DP_LCD_OUT_BT656_BITSEL(reg0, DP_ON);
		D4_DP_LCD_OUT_BT656_CBCR_SEL(reg0, DP_OFF);
		break;

	case LCD_OUT_BT656_10BIT_CrYCbY:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_ON);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_ON);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_24);
		D4_DP_LCD_OUT_BT656_BITSEL(reg0, DP_ON);
	...
	case LCD_OUT_FUJITU_8BIT_CbYCrY:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_OFF);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_ON);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_FUJITU_24);
		D4_DP_LCD_OUT_YCC_SEQ(reg0, 0);
		break;

	case LCD_OUT_FUJITU_8BIT_CrYCbY:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_OFF);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_ON);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_FUJITU_24);
		D4_DP_LCD_OUT_YCC_SEQ(reg0, 1);
		break;
	case LCD_OUT_FUJITU_8BIT_YCbYCr:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_OFF);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_ON);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_FUJITU_24);
		D4_DP_LCD_OUT_YCC_SEQ(reg0, 2);
		break;

	case LCD_OUT_FUJITU_8BIT_YCrYCb:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_OFF);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_ON);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_FUJITU_24);
		D4_DP_LCD_OUT_YCC_SEQ(reg0, 3);
		break;

	case LCD_OUT_FUJITU_RGB_3TIMES:
		D4_DP_LCD_OUT_MULTI_CLK_TRANS(reg1, DP_ON);
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_OFF);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_OFF);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_FUJITU_24);
		break;

	case LCD_OUT_FUJITU_RGB_DUMMY:
		D4_DP_LCD_OUT_MULTI_CLK_TRANS(reg1, DP_ON);
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_OFF);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_OFF);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_FUJITU_8);
		break;
	case LCD_OUT_FUJITU_RGB565:
		D4_DP_LCD_OUT_BT656_ON(reg0, DP_OFF);
		D4_DP_LCD_OUT_CTRL_YCBCR(reg0, DP_OFF);
		D4_DP_LCD_OUT_BUS_WIDTH(reg0, DATA_FUJITU_8);
		break;
...
{
	struct stdp_display_area display = {0, };
...
int dp_slcd_ISR_pending_clear(void)
{
	...
	if (dp_get_interrupt_status(&lcd_info, INT_SLCD)) {
...
unsigned int d4_dp_lcd_graphic_address_get(enum edp_window window)
{
	unsigned int base_offset = 0;
	...
	return __raw_readl(lcd_info.dp_lcd_regs + base_offset);
}
 ...
 * @note
 */
void d4_dp_lcd_interrupt_onoff(enum edp_onoff onoff, enum einterrupt_type sel, int arg)
{
	DpPRINTK("%s(),%d,onoff =%d, sel\n", __FUNCTION__, __LINE__, onoff, sel);

	if (onoff == DP_ON) {
		g_dp_lcd_interrupt_state |= (1<<sel);
		g_dp_lcd_interrupt_arg[sel] = arg;
		dp_interrupt_on_off(&lcd_info, INT_MLCD, onoff);
	} else {
		g_dp_lcd_interrupt_state &= ~(1<<sel);
		g_dp_lcd_interrupt_arg[sel] = 0;

		if (g_dp_lcd_interrupt_state == 0) {
			dp_interrupt_on_off(&lcd_info, INT_MLCD, onoff);
		}
	}
}

int d4_dp_lcd_interrupt_state()
{
	return g_dp_lcd_interrupt_state;
}

int d4_dp_lcd_interrupt_arg(enum einterrupt_type sel)
{
	return g_dp_lcd_interrupt_arg[sel];
}

void d4_dp_sublcd_interrupt_onoff(enum edp_onoff onoff)
{
	DpPRINTK("%s(),%d,onoff =%d\n", __FUNCTION__, __LINE__, onoff);
	dp_interrupt_on_off(&lcd_info, INT_SLCD, onoff);
}
...
{
	printk("pos x = %d, y = %d, w = %d, h = %d \n", bb_onoff->area.H_Start, 
		bb_onoff->area.V_Start, bb_onoff->area.H_Size, bb_onoff->area.V_Size);

	if(lcd_pannel.h_size == 480)
	{
		int swap_size, swap_start;
		swap_size = bb_onoff->area.H_Size;
		swap_start = bb_onoff->area.H_Start;
		bb_onoff->area.H_Size = bb_onoff->area.V_Size;
		bb_onoff->area.H_Start = (480 - (bb_onoff->area.V_Start + bb_onoff->area.V_Size));
		bb_onoff->area.V_Size = swap_size;
		bb_onoff->area.V_Start = swap_start;
	}
 ...
 * @fn int d4_sublcd_video_init_display(struct stfb_video_info *video)
 * @param void
 * @return
 *
 * @note
...
{
	/* video stride set */
	dp_sublcd_set_stride(&lcd_info, DP_VIDEO, video->vid_stride);
	/* video scantype set: default progressive */
	dp_sublcd_set_scantype(&lcd_info);

	/* video format set : default ycbcr 420 */
	dp_sublcd_set_path_onoff(&lcd_info, LCD_CTRL_YCBCR422, DP_OFF);

	/* video image address set  */
	dp_sublcd_set_vid_image_address(&lcd_info, video);

	/* video display area set: default wind0,8bit */
	dp_sublcd_set_vid_display_area(&lcd_info, DP_WIN0, _8bit, &(video->display));
	/* lcd video window on */
		...
		enum edp_layer layer, enum elcd_csc_type type)
{
	...
	if (DP_VIDEO == layer) {
		...
		dp_sublcd_set_path_onoff(info, LCD_CTRL_RGB2YCBCR_ON, DP_ON);
		switch (type) {
		case LCD_CSC_SD_STANDARD:
			D4_DP_LCD_YCC2RGB_MTRX0(CSC_Matrix_00_01, 0x06d);
			D4_DP_LCD_YCC2RGB_MTRX1(CSC_Matrix_00_01, 0x16e);
...
void d4_dp_bt656_onoff(struct stlcd_bt656 val)
{
	if (val.onoff) {
		if (val.bit == _8bit) {
			if (val.ycbcr_order == CB_Y_CR_Y) {
				if (val.path == DP_MLCD)
					dp_lcd_out_type(&lcd_info, LCD_OUT_BT656_8BIT_CbYCrY);
				if (val.path == DP_SLCD)
					dp_sublcd_out_type(&lcd_info, LCD_OUT_BT656_8BIT_CbYCrY);
			} else {
				if (val.path == DP_MLCD)
					dp_lcd_out_type(&lcd_info, LCD_OUT_BT656_8BIT_CrYCbY);
				if (val.path == DP_SLCD)
					dp_sublcd_out_type(&lcd_info, LCD_OUT_BT656_8BIT_CrYCbY);
			}
		} else {
			if (val.ycbcr_order == CB_Y_CR_Y) {
				if (val.path == DP_MLCD)
					dp_lcd_out_type(&lcd_info, LCD_OUT_BT656_10BIT_CbYCrY);
				if (val.path == DP_SLCD)
					dp_sublcd_out_type(&lcd_info, LCD_OUT_BT656_10BIT_CbYCrY);
			} else {
				if (val.path == DP_MLCD)
					dp_lcd_out_type(&lcd_info, LCD_OUT_BT656_10BIT_CrYCbY);
				if (val.path == DP_SLCD)
					dp_sublcd_out_type(&lcd_info, LCD_OUT_BT656_10BIT_CrYCbY);
			}
		}

		/**< Set CSC for RGB2YCC */
		if (val.path == DP_MLCD) {
			dp_lcd_set_csc_matrix(&lcd_info, DP_GRP, LCD_CSC_HD_STANDARD);
		} else if (val.path == DP_SLCD) {
			dp_sublcd_set_csc_matrix(&lcd_info, DP_GRP, LCD_CSC_HD_STANDARD);
		}

	} else {
		dp_lcd_out_type(&lcd_info, lcd_out_type);
	}
}
...
void dp_lcd_set_manual_csc_matrix(struct stfb_info *info, struct stlcd_csc_matrix *matrix)
{
	unsigned int CSC_Matrix_Y_Cb_offset = 0;
	unsigned int CSC_Matrix_Cr_offset = 0;
	unsigned int CSC_Matrix_00_01 = 0;
	...
	D4_DP_LCD_YCC2RGB_MTRX0(CSC_Matrix_00_01, matrix->matrix_set_00);
	D4_DP_LCD_YCC2RGB_MTRX1(CSC_Matrix_00_01, matrix->matrix_set_01);

	D4_DP_LCD_YCC2RGB_MTRX0(CSC_Matrix_02_10, matrix->matrix_set_02);
	D4_DP_LCD_YCC2RGB_MTRX1(CSC_Matrix_02_10, matrix->matrix_set_10);

	D4_DP_LCD_YCC2RGB_MTRX0(CSC_Matrix_11_12, matrix->matrix_set_11);
	D4_DP_LCD_YCC2RGB_MTRX1(CSC_Matrix_11_12, matrix->matrix_set_12);

	D4_DP_LCD_YCC2RGB_MTRX0(CSC_Matrix_20_21, matrix->matrix_set_20);
	D4_DP_LCD_YCC2RGB_MTRX1(CSC_Matrix_20_21, matrix->matrix_set_21);

	D4_DP_LCD_YCC2RGB_MTRX0(CSC_Matrix_22, matrix->matrix_set_22);

	D4_DP_LCD_YCC2RGB_MTRX0(CSC_Matrix_Y_Cb_offset, matrix->ycbcr_offset_y);
	D4_DP_LCD_YCC2RGB_MTRX1(CSC_Matrix_Y_Cb_offset, matrix->ycbcr_offset_cb);

	D4_DP_LCD_YCC2RGB_MTRX0(CSC_Matrix_Cr_offset, matrix->ycbcr_offset_cr);

	D4_DP_LCD_YCC2RGB_UNDER_RGB(CSC_Matrix_range, matrix->rgb_lower_range);
	D4_DP_LCD_YCC2RGB_OVER_RGB(CSC_Matrix_range, matrix->rgb_upper_range);
	...
	__raw_writel(CSC_Matrix_Y_Cb_offset, info->dp_lcd_regs + DP_GRP_CSC5);
	__raw_writel(CSC_Matrix_Cr_offset, info->dp_lcd_regs + DP_GRP_CSC6);
	__raw_writel(CSC_Matrix_range, info->dp_lcd_regs + DP_GRP_CSC7);
	__raw_writel(CSC_Matrix_range, info->dp_lcd_regs + DP_GRP_CSC8);
}

void lcd_csc_manual_set(struct stlcd_csc_matrix *matrix)
{
#if defined(CONFIG_MACH_D4_NX300) || defined(CONFIG_MACH_D4_NX2000)
	dp_lcd_set_path_onoff(&lcd_info, LCD_CTRL_RGB2YCBCR_ON, DP_ON);
    /* tcc set*/
	dp_lcd_set_path_onoff(&lcd_info, LCD_CTRL_TCC_ON, DP_ON);
#endif
	
	dp_lcd_set_manual_csc_matrix(&lcd_info, matrix);
}
...
void drime4_lcd_display_init(void)
{
	...
	dp_lcd_grp_set_ctrl_onoff(&lcd_info, GRP_CTRL_ARGB_ORDER, DP_ON);

	/*flat alpha on */
	dp_lcd_set_grp_flat_alpha_onoff(&lcd_info, DP_WIN0, 0xff, 0);
...
{
	lcd_pannel.even_seq = pannel->even_seq;
	lcd_pannel.odd_seq = pannel->odd_seq;
	lcd_pannel.h_size = pannel->h_size;
	lcd_pannel.v_size = pannel->v_size;
	lcd_pannel.lcd_data_width = pannel->lcd_data_width;
	lcd_pannel.type = pannel->type;
	lcd_pannel.timing = pannel->timing;
	dp_lcd_pannel_set();

	if (lcd_pannel.pixclock == 0)
		lcd_pannel.pixclock = 27000000;
}
...
{
	sublcd_pannel.even_seq = pannel->even_seq;
	sublcd_pannel.odd_seq = pannel->odd_seq;
	sublcd_pannel.h_size = pannel->h_size;
	sublcd_pannel.v_size = pannel->v_size;
	sublcd_pannel.lcd_data_width = pannel->lcd_data_width;
	sublcd_pannel.type = pannel->type;
	sublcd_pannel.timing = pannel->timing;
	dp_sublcd_pannel_set();

	if (sublcd_pannel.pixclock == 0)
		sublcd_pannel.pixclock = 27000000;
}
...
void dp_k_dma_rmu_bus(void)
{
	/* DMA Start */
	__raw_writel(0x80, lcd_info.dp_global_regs + 0x1444);
	__raw_writel(0x80000000, lcd_info.dp_global_regs + 0x143C);
	__raw_writel(0x0, lcd_info.dp_global_regs + 0x1454);
	__raw_writel(0x0010000, lcd_info.dp_global_regs + 0x1448);
...
static void dp_k_dma_sw_reset(void)
{
	/* DMA Reset & INT Clear */
	__raw_writel(0xFFFFFFFF, lcd_info.dp_global_regs + 0x8);
	__raw_writel(0x0, lcd_info.dp_global_regs + 0x8);
	__raw_writel(0xFFFFFFFF, lcd_info.dp_global_regs + 0x14);
}


int dp_pmu_requeset(void)
{
	int ret = 0;
#ifdef CONFIG_PMU_SELECT
	/* DMA Start */
	__raw_writel(0x1, lcd_info.dp_global_regs + 0x145C);
	ret = pmu_wait_for_ack(PMU_DP);
	if (ret != 0) {
		return ret;
	}
#endif
...
void dp_pmu_clear(void)
{
	__raw_writel(0x0, lcd_info.dp_global_regs + 0x145C);
}

void dp_pmu_on_off(enum edp_onoff onoff)
{
	int reval;
	struct clk *mlcd_clk, *mlcd_out_clk, *mlcd_sel, *bus_clk, *lcd_clk;
	struct d4_rmu_device *rmu;

	/* mlcd path set */
	mlcd_clk = clk_get(NULL, "mlcd");
	if ((int)mlcd_clk == -2)
		return;

	mlcd_out_clk = clk_get(NULL, "mlcd_out");
	if ((int)mlcd_out_clk == -2)
		return;

	mlcd_sel = clk_get(NULL, "mlcd_sel");
	if ((int)mlcd_sel == -2)
		return;

	/* lcd bus clock set */
	bus_clk = clk_get(NULL, "dp");
	if ((int)bus_clk == -2)
		return;

	/* mlcd clock set */
	lcd_clk = clk_get(NULL, "mlcd_sel1");
		if ((int)lcd_clk == -2)
		return;

	if (onoff == DP_ON) {
		reval = d4_pmu_check(PMU_DP);
		if (reval != 0) {
			reval = d4_pmu_request(PMU_DP);
			if (reval)
				return;
			...
			d4_pmu_isoen_set(PMU_DP, PMU_CTRL_ON);
			d4_sw_isp_reset(rmu, RST_DP);

			clk_disable(mlcd_clk);
			clk_put(mlcd_clk);

			clk_disable(mlcd_out_clk);
			clk_put(mlcd_out_clk);

/*
			clk_disable(mlcd_sel);
			clk_put(mlcd_sel);
*/
			...
			d4_pmu_request_clear(PMU_DP);

			d4_pmu_scpre_set(PMU_DP, PMU_CTRL_ON);
			d4_pmu_scall_set(PMU_DP, PMU_CTRL_ON);
			d4_rmu_release(rmu);
		}
	} else {
		d4_pmu_scpre_set(PMU_DP, PMU_CTRL_OFF);
		d4_pmu_scall_set(PMU_DP, PMU_CTRL_OFF);
		wait_for_stable(PMU_DP);
		...
		clk_set_rate(bus_clk, 162000000); /* bus clock */
		lcd_clk = clk_get(NULL, "mlcd_sel1");
		if ((int)lcd_clk == -2)
			return;
		clk_enable(lcd_clk);

		clk_set_rate(lcd_clk, 27000000);  /* lcd clock */

		d4_pmu_isoen_set(PMU_DP, PMU_CTRL_OFF);

		udelay(1);
		d4_sw_isp_reset_release(rmu, RST_DP);

		/*2~4*/
		d4_pmu_bus_reset(PMU_DP);
		dp_k_dma_rmu_bus();
...
void d4_dp_mlcd_clock_ctrl(unsigned int mlcdclock)
{
	...
	if ((int)lcd_clk == -2)
		return;
	clk_set_rate(lcd_clk, mlcdclock); /* lcd clock */
	lcd_pannel.pixclock = mlcdclock;
}

void d4_dp_slcd_clock_ctrl(unsigned int slcdclock)
{
	struct clk *lcd_clk;
	lcd_clk = clk_get(NULL, "slcd_sel1");
	if ((int)lcd_clk == -2)
		return;

	clk_set_rate(lcd_clk, slcdclock); /* lcd clock */
	sublcd_pannel.pixclock = slcdclock;
}

void dp_bus_clock_change(unsigned long isHighRate)
{
//	d4_dp_lcd_interrupt_onoff(DP_ON, BUSCLOCK_CHANGE_INTERRUPT, isHighRate);
}

#ifdef CONFIG_PM
static struct lcd_sleep_save lcd_save[] = { 
	{ DP_LCD_PATH_CTRL, 0 }, 
	{ DP_LCD_SIZE, 0 }, 
	{ DP_LCD_STRIDE, 0 }, 
	{ DP_LCD_DMA_MODE2, 0 }, 
...
#if 0
	{ DP_LCD_WIN0_F0_Y_ADDR, 0 }, 
	{ DP_LCD_WIN0_F0_C_ADDR, 0 }, 
	{ DP_LCD_WIN0_F1_Y_ADDR, 0 }, 
	{ DP_LCD_WIN0_F1_C_ADDR, 0 }, 
	{ DP_LCD_WIN0_H_POS, 0 }, 
	{ DP_LCD_WIN0_V_POS, 0 },
#endif		

	{ DP_LCD_GRP1_DMA_CTRL, 0 }, 
	{ DP_LCD_GRP1_DMA_SCALER, 0 }, 
	{ DP_LCD_GRP1_BKG, 0 }, 
	{ DP_LCD_GRP1_WIN_FLAT_ALPHA, 0 },
	{ DP_LCD_GRP1_WIN_PRIORITY, 0 },

	{ DP_LCD_GRP1_WIN0_F0_ADDR, 0 }, 
	{ DP_LCD_GRP1_WIN0_F1_ADDR, 0 }, 
	{ DP_LCD_GRP1_WIN0_H_POS, 0 }, 
	{ DP_LCD_GRP1_WIN0_V_POS, 0 },

	{ DP_LCD_GRP1_WIN1_F0_ADDR, 0 }, 
	{ DP_LCD_GRP1_WIN1_F1_ADDR, 0 }, 
	{ DP_LCD_GRP1_WIN1_H_POS, 0 }, 
	{ DP_LCD_GRP1_WIN1_V_POS, 0 },
	{ DP_LCD_GRP1_WIN2_F0_ADDR, 0 }, 
	{ DP_LCD_GRP1_WIN2_F1_ADDR, 0 }, 
	{ DP_LCD_GRP1_WIN2_H_POS, 0 }, 
	{ DP_LCD_GRP1_WIN2_V_POS, 0 },

	{ DP_LCD_GRP1_WIN3_F0_ADDR, 0 }, 
	{ DP_LCD_GRP1_WIN3_F1_ADDR, 0 }, 
	{ DP_LCD_GRP1_WIN3_H_POS, 0 }, 
	{ DP_LCD_GRP1_WIN3_V_POS, 0 } 
};
...
int backup_interrupt_state;
int is_lcd_reg_saved;
/**
 * @brief dp lcd register save function in suspend mode
 * @fn void dp_lcd_save_reg(void)
 * @param *info[in]  stfb_info reference linux/include/vide/drime4/d4_dp_type.h
 * @return
 *
 * @note
...
void dp_lcd_save_reg(void)
{
	...
	if (is_lcd_reg_saved == 0) {
		...
		dp_lcd_grp_window_onoff(&lcd_info, DP_WIN2, DP_OFF);
		dp_lcd_grp_window_onoff(&lcd_info, DP_WIN3, DP_OFF);	
		dp_lcd_do_save(&lcd_info, lcd_save, ARRAY_SIZE_TEST(lcd_save));
		backup_interrupt_state = __raw_readl(lcd_info.dp_global_regs + dp_int_on);
		backup_interrupt_state &= 0xF0;
		is_lcd_reg_saved = 1;
	}
}
 ...
 * @fn void dp_lcd_restroe_reg(void)
 * @param *info[in]  stfb_info reference linux/include/vide/drime4/d4_dp_type.h
 * @return
 *
 * @note
...
{
	DpPRINTK("%s(),%d\n", __FUNCTION__, __LINE__);
	dp_lcd_do_restore(&lcd_info, lcd_save, ARRAY_SIZE_TEST(lcd_save));
	__raw_writel(backup_interrupt_state, lcd_info.dp_global_regs + dp_int_on);
	is_lcd_reg_saved = 0;
}
#endif
...
{
	struct clk *mlcd_clk, *mlcd_out_clk;
